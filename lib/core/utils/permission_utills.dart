...
Future<Position?> handleLocationPermission(BuildContext context) async {
  ...
  if (status.isDenied) {
    ...
    if (result.isGranted) {
      ...
      // Just denied again
      _showSnackBar(context, AppLocalizations.of(context)!.locationPermissionPermanentlyDenied);
...
Future<Position?> fetchCurrentLocation(BuildContext context) async {
  ...
  try {
    ...
    context.read<WeatherCubit>().setCurrentLocation(position.latitude,position.longitude, city);

    await HomePage.toRoute(context: context);
  } catch (e) {
...
void _showPermissionDeniedSnackBar(BuildContext context) {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      ...
      content: AppText(appStrings.locationPermissionPermanentlyDenied, style: appStyles.s14w400Grey, maxLines: 2),
...
void _showLocationServicesDialog(BuildContext context) {
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      title: CenterText(appStrings.locationPermissionNeeded, style: appStyles.s16w500black),
      content: Text(AppLocalizations.of(context)!.locationPermissionNeededDis, style: appStyles.s14w400Grey),
      actions: [
        AppButton(
          onPressed: () async {
...
void _showSnackBar(BuildContext context, String message) {
  ...
  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: AppText(message, style: AppStyles.of(context).s14w400Grey)));
}
