...
class MapPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: GoogleMap(
      ...
      onTap: (latLng) => context.read<WeatherCubit>().setCurrentLocation(latLng.latitude, latLng.longitude,'h'),
      tileOverlays: {
        TileOverlay(
          tileOverlayId: TileOverlayId('precipitation'),
          tileProvider: UrlTileProvider(
            urlTemplate: "https://tile.openweathermap.org/map/precipitation_new/{z}/{x}/{y}.png?appid=${AppConfig.apiKey}",
          ),
        ),
        TileOverlay(
          tileOverlayId: TileOverlayId('temperature'),
          tileProvider: UrlTileProvider(
            urlTemplate: "https://tile.openweathermap.org/map/temp_new/{z}/{x}/{y}.png?appid=${AppConfig.apiKey}",
          ),
        ),
      },
    ));
